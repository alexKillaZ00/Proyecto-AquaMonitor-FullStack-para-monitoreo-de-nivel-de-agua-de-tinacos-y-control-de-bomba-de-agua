...
const form = document.getElementById('addBombaForm')
...
function validateForm() {
  ...
  // Validaciones basadas en validarRegistroBomba
  if (!nombre || nombre.length < 2 || nombre.length > 100) {
    errNombre.textContent = 'El nombre de la bomba debe tener entre 2 y 100 caracteres'
    ok = false
  }

  if (ubicacion) {
    if (ubicacion.length < 2 || ubicacion.length > 100) {
      errUbicacion.textContent = 'La ubicacion de la bomba debe tener entre 2 y 100 caracteres'
      ok = false
    }
  }
  ...
  if (!codigo) {
    ...
    ok = false
  } else if (codigo.length < 11 || codigo.length > 15) {
...
async function submitCreate(ev) {
  ...
  try {
    const res = await authenticatedFetch(`${API_BASE_URL}/bombas`, { method: 'POST', body: JSON.stringify(payload) })
    if (!res.ok) {
      const txt = await res.text().catch(() => '')
      throw new Error(txt || `Error al registrar bomba (${res.status})`)
    }

    formGlobalSuccess.textContent = 'Bomba registrada correctamente'
